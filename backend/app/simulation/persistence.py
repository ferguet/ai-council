...
def world_to_dict(world: WorldState) -> dict:
    # Calcular ánimo colectivo: promedio de happiness y anger de todos los ciudadanos
    from app.domain.city_models import mood_label_for

    if world.citizens:
        avg_happiness = sum(c.happiness for c in world.citizens.values()) / len(world.citizens)
        avg_anger = sum(c.anger for c in world.citizens.values()) / len(world.citizens)
        collective_mood = mood_label_for(int(avg_happiness), int(avg_anger))
    else:
