"""
Proveedor para Cerebras Inference (chips especializados, muy rapidos). API
compatible con el formato OpenAI. Nivel gratuito muy generoso: 1 millon de
tokens al dia, sin tarjeta, solo con email.

Cuenta gratis en https://cloud.cerebras.ai
        ...
        async with httpx.AsyncClient(timeout=60) as client:
            async with client.stream("POST", _URL, headers=self._headers(), json=payload) as resp:
                if resp.status_code != 200:
                    ...
                    raise ProviderError(f"Cerebras error {resp.status_code}: {body[:300]!r}")
