...
class GLMProvider(AIProvider):
    ...
    async def chat(self, messages: list[ChatMessage], model: str, temperature: float = 0.7) -> str:
        if not self._api_key:
            raise ProviderError("GLM_API_KEY no configurada")
        payload = {
            "model": model,
        ...
        if resp.status_code != 200:
            raise ProviderError(f"GLM error {resp.status_code}: {resp.text[:300]}")
        data = resp.json()
        ...
        async with httpx.AsyncClient(timeout=60) as client:
            async with client.stream("POST", _URL, headers=self._headers(), json=payload) as resp:
                if resp.status_code != 200:
                    ...
                    raise ProviderError(f"GLM error {resp.status_code}: {body[:300]!r}")
