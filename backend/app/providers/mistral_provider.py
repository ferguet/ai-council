"""
Proveedor para Mistral AI (La Plateforme). API compatible con el formato
OpenAI. Nivel gratuito "Experiment": acceso limitado por velocidad a todos
los modelos, sin tarjeta, pensado para pruebas (no produccion a gran escala,
pero de sobra para un ciudadano que piensa cada 15 min).

Cuenta gratis en https://console.mistral.ai (registro con email).
Doc: https://docs.mistral.ai/api/endpoint/chat
"""
        ...
        async with httpx.AsyncClient(timeout=60) as client:
            async with client.stream("POST", _URL, headers=self._headers(), json=payload) as resp:
                if resp.status_code != 200:
                    ...
                    raise ProviderError(f"Mistral error {resp.status_code}: {body[:300]!r}")
