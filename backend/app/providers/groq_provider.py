"""
Proveedor para Groq (API compatible con el formato OpenAI). Nivel gratuito
generoso e inferencia muy rapida sobre modelos abiertos (Llama, Mixtral...).
        ...
        async with httpx.AsyncClient(timeout=60) as client:
            async with client.stream("POST", _URL, headers=self._headers(), json=payload) as resp:
                if resp.status_code != 200:
                    ...
                    raise ProviderError(f"Groq error {resp.status_code}: {body[:300]!r}")
